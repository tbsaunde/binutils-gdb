...
#define is_br_jmp_insn_p(op)    (((op)->klass == BRANCH || (op)->klass == JUMP))
#define is_kernel_insn_p(op)    (((op)->klass == KERNEL))
		   ...
		   int *pcpumatch)
{
  ...
  for (opcode = arc_opcode_hash_entry_iterator_next (entry, &iter);
      ...
      for (flgidx = opcode->flags; *flgidx; ++flgidx)
	{
	  ...
	  if ((cl_flags->klass & F_CLASS_REQUIRED) && cl_matches == 0)
	    goto match_failed;
	  if ((cl_flags->klass & F_CLASS_OPTIONAL) && cl_matches > 1)
	    goto match_failed;
	}
  ...
  for (argidx = opcode->operands; *argidx; ++argidx)
	...
	default:
	  ...
	  switch (t->X_md)
	    ...
	    case O_pcl:
	      reloc = ARC_RELOC_TABLE (t->X_md)->reloc;
	      if (ARC_SHORT (opcode->mask) || opcode->klass == JUMP)
		as_bad_where (frag_now->fr_file, frag_now->fr_line,
